...
export async function POST(request: Request) {
  ...
  if (!booking_id || !agreed_amount) {
    return NextResponse.json(
      { error: "booking_id dan agreed_amount wajib diisi." },
      { status: 400 },
    );
  }
  ...
  try {
    ...
    await conn.query(
      `CALL sp_create_transaction(?, ?, ?, ?, ?, ?, ?, ?, @transaction_id)`,
      [
        booking_id,
        agreed_amount,
        transfer_date ?? null,
 ...
 * Update status transaksi — trigger trg_transaction_success saat status='success'
 */
export async function PATCH(request: Request) {
  ...
  const [rows] = await pool.execute<RowDataPacket[]>(
    `SELECT id FROM transaction WHERE id = ? AND agent_id = ? LIMIT 1`,
  ...
  await pool.execute(`UPDATE transaction SET status = ? WHERE id = ?`, [
    status,
    transaction_id,
  ]);
